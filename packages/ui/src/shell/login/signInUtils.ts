...
export function startSignInWithoutTenant(provider: RedirectProviderId, redirectTo?: string): void {
    ...
    localStorage.removeItem('tenantName');
    auth.tenantId = Env.firebase?.tenantId ?? null;
    const idp = Env.firebase?.tenantId ? ((Env.firebase.providerType as ProviderId) ?? 'oidc') : provider;
    // A password tenant has nothing to redirect to, and without an email there is no password step
    // to offer; its users sign in through email discovery instead.
    if (idp === 'password') return;
    void signInWithRedirect(auth, buildFirebaseProvider(idp, undefined, redirectTo));
}
