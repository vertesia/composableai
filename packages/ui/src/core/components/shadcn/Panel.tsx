...
export function Panel({
    children, action, title, description, footer, className,
    collapsible, defaultCollapsed = false, size = 'md', clickableHeader = false,
}: PanelProps) {
    ...
    const headerInner = (
        <>
            <div className="flex items-center gap-2">
                <div className={`font-semibold ${isSmall ? 'text-sm' : 'text-lg'}`}>
                    {title}
                </div>
            ...
            <div className="flex gap-2 items-center">
                ...
                {collapsible && (
                    useClickableHeader
                        ? chevron
                        : <Button variant="ghost" size="icon" onClick={() => setIsCollapsed(!isCollapsed)}>{chevron}</Button>
                )}
            </div>
        </>
    );
    ...
    return (
        <div className={`flex flex-col ${sectioned ? '' : 'p-4 gap-2'} rounded-sm border bg-card ${className ?? ''}`}>
            {useClickableHeader ? (
                <button
                    type="button"
