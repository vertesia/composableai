...
function RadioGroupItem({ className, ...props }: React.ComponentProps<typeof RadioGroupPrimitive.Item>) {
    return (
        <RadioGroupPrimitive.Item
            ...
            className={cn(
                'border-input dark:bg-input/30 data-checked:bg-primary data-checked:text-primary-foreground dark:data-checked:bg-primary data-checked:border-primary aria-invalid:aria-checked:border-primary aria-invalid:border-destructive-foreground focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive-foreground/20 dark:aria-invalid:ring-destructive-foreground/40 dark:aria-invalid:border-destructive-foreground/50 flex size-4 rounded-full focus-visible:ring-3 aria-invalid:ring-3 group/radio-group-item peer relative aspect-square shrink-0 border outline-none after:absolute after:-inset-x-3 after:-inset-y-2 disabled:cursor-not-allowed disabled:opacity-50',
                className,
            )}
            {...props}
        >
...
export abstract class RadioGroupAdapter<T> {
    ...
    descriptionOf(_item: T): React.ReactNode {
        return undefined;
    }
    selectedClassName(_item: T): string {
...
function RadioGroup<T = RadioGroupOption>({ onSelect, selected, options, adapter }: RadioGroupProps<T>) {
    ...
    const getDescription = (o: T) =>
        adapter ? adapter.descriptionOf(o) : (o as unknown as RadioGroupOption).description;
