...
function statusBadgeClass(status?: ToolExecutionStatus): string {
    switch (status) {
        ...
        case 'warning':
            return 'bg-attention text-attention border-attention';
        default:
            return '';
    }
}
...
export type ToolCallGroupClassNames = Partial<
    Pick<
        ToolCallGroupProps,
        | 'rootClassName'
        | 'headerClassName'
...
const getFilesFromDetails = (
    details: { files?: string[]; outputFiles?: string[]; [key: string]: unknown } | undefined,
): string[] | undefined => {
...
const TOOL_BADGE_CLASS = 'text-[10px] px-1.5 py-0.5 rounded-md bg-done text-done border border-done font-medium';
const ASSISTANT_BADGE_CLASS =
    'text-[10px] px-1.5 py-0.5 rounded-md bg-muted text-foreground border border-border font-medium';
...
function ToolCallItem({ message, isExpanded, onToggle, artifactRunId, classNames = {} }: ToolCallItemProps) {
    ...
    const details = message.details as
        | { tool?: string; files?: string[]; outputFiles?: string[]; [key: string]: unknown }
        | undefined;
    ...
    const copyToClipboard = () => {
        const textToCopy = [messageContent, details ? `\n\nDetails:\n${JSON.stringify(details, null, 2)}` : '']
            .join('')
            .trim();
    ...
    return (
        <div className={cn('border-b border-border last:border-b-0', classNames.itemClassName)}>
            ...
            <div
                ...
                tabIndex={0}
                className={cn(
                    'flex items-start justify-between px-3 py-2 cursor-pointer hover:bg-muted transition-colors',
                    classNames.itemHeaderClassName,
                )}
                onClick={onToggle}
            ...
            >
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {!isExpanded && <span className={cn(badgeClass, classNames.toolBadgeClassName)}>{toolName}</span>}
                    <Button
                        variant="ghost"
                        size="xs"
                        onClick={(e) => {
            ...
            {isExpanded &&
                (() => {
                    const toolStatusValue = (details as Record<string, unknown> | undefined)?.tool_status as
                        | ToolExecutionStatus
                        | undefined;
                                ...
                                {toolStatusValue && (
                                    <span
                                        className={cn(
                                            ...
                                            statusBadgeClass(toolStatusValue),
                                        )}
                                    >
                                        {toolStatusValue}
                                    </span>
...
function ToolCallGroupComponent({
    ...
    const renderStatusIndicator = () => {
        ...
        if (toolStatus === 'running') {
            return <span className="size-2 rounded-full bg-primary inline-block" />;
        }
        ...
        return <Bot className="size-4 text-done" />;
    };
    ...
    return (
        <div className={cn('border-s-4 overflow-hidden bg-white dark:bg-muted mb-4', getBorderColor(), rootClassName)}>
            ...
            <div
                ...
                className={cn(
                    'flex items-center justify-between px-4 py-1.5 cursor-pointer hover:bg-muted transition-colors',
                    headerClassName,
                )}
            ...
            >
                <div className="flex items-center gap-1 flex-1 min-w-0">
                    ...
                    {preambleText ? (
                        <span
                            className={cn('text-xs text-foreground line-clamp-2 flex-1 min-w-0', toolSummaryClassName)}
                        >
                            {preambleText}
                        </span>
                    ) : (
                        <>
                            ...
                            <span className={cn('text-xs text-done font-medium', toolSummaryClassName)}>
                                {toolSummary}
                            </span>
                        </>
                    )}
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                    {preambleText && (
                        <span className={cn('text-[10px] text-done font-medium', toolSummaryClassName)}>
                            {toolSummary}
                        </span>
                    )}
                ...
                <div className="px-3 py-0.5 space-y-0">
                    {messages.map((m, idx) => {
                        ...
                        return (
                            <div
                                key={itemKey}
                                className={cn('border-b border-border last:border-b-0', itemClassName)}
                                style={{
                            ...
                            >
                                ...
                                <div
                                    ...
                                    className={cn(
                                        'flex items-start gap-2 py-1.5 text-xs cursor-pointer hover:bg-muted',
                                        itemHeaderClassName,
                                    )}
                                ...
                                {isItemExpanded &&
                                    (() => {
                                        const toolStatusValue = (details as Record<string, unknown> | undefined)
                                            ?.tool_status as ToolExecutionStatus | undefined;
                                        const interestingDetails = extractInterestingDetails(
                                            details as Record<string, unknown> | undefined,
                                        );
