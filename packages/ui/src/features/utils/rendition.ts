...
export async function retrieveRendition(
    ...
    if (!doc?.content?.type || !(isImage(doc.content.type) || isVideo(doc.content.type) || isPdf(doc.content.type))) {
    ...
    const savedId = localStorage.getItem(`${RENDITION_ID}_${doc.id}_${RENDITION_OPTIONS.max_hw}`);
    const elapsedTime = localStorage.getItem(`${RENDITION_ID}_${doc.id}_${RENDITION_OPTIONS.max_hw}_time`);

    if (savedId?.length && elapsedTime && Math.abs(currentTime - parseInt(elapsedTime, 10)) <= 900) {
    ...
    setRenditionStatus('Preparing preview...');
    client.objects
        .getRendition(doc.id, RENDITION_OPTIONS)
        .then((response) => {
            if (response.status === 'generating') {
                ...
                setRenditionAlt('');
                setTimeout(
                    () => retrieveRendition(client, doc, setRenditionUrl, setRenditionAlt, setRenditionStatus),
                    60000,
                );
                ...
                localStorage.setItem(`${RENDITION_ID}_${doc.id}_${RENDITION_OPTIONS.max_hw}`, rendition);
