    ...
    return (
        <Table className="w-full">
            ...
            <TBody isLoading={isLoading} columns={columns.length + 1}>
                        ...
                        >
                            {selection && (
                                ...
                                <td onClick={(ev) => ev.stopPropagation()}>
                                    <input
                                        checked={selection.isSelected(obj.id)}
                                        type="checkbox"
                                        className={`${!selection.isSelected(obj.id) ? 'hidden group-hover:block' : ''}`}
                ...
                {objects.length === 0 && (
                    <tr>
                        <td colSpan={columns.length + (selection ? 1 : 0)} className="text-center">
