...
function AddToCollectionForm({ onClose, objectIds }: AddToCollectionFormProps) {
    ...
    const onAddToCollection = ({ collectionId }: { collectionId: string }) => {
        ...
        client.store.collections
            .addMembers(collectionId, objectIds)
            .then(() => {
                toast({
                ...
                <div className="p-4 min-w-0 max-w-full overflow-hidden">
                    ...
                    <ModalFooter>
                        <Button
                            ...
                            onClick={() =>
                                selectedCollectionId && onAddToCollection({ collectionId: selectedCollectionId })
                            }
                        >
            ...
            content: (
                <div className="p-4">
                    <CreateCollectionForm
                        onClose={onClose}
                        onAddToCollection={(id: string) => onAddToCollection({ collectionId: id })}
                        redirect={false}
                    />
                </div>
            ),
