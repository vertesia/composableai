    ...
    const onCreate = () => {
        ...
        client.store.collections
            ...
            .then((r) => {
                ...
                if (redirect) navigate(`/collections/${r.id}`);
                ...
                <form onSubmit={(e) => e.preventDefault()}>
                    <FormItem label={t('type.name')} required>
                        <Input
                            type="text"
                            value={payload.name || ''}
                            onChange={(value) => setPayloadProp('name', value)}
                        />
                    </FormItem>
                    ...
                    <FormItem
                        label={t('store.dynamicCollection')}
                        className="mt-2"
                        direction="row"
                        description={t('store.dynamicCollectionDescription')}
                    >
                        <Switch
                            value={payload.dynamic || false}
                            onChange={(value) => setPayloadProp('dynamic', value)}
                        />
                    </FormItem>
                    {!payload.dynamic && (
                        <FormItem
                            ...
                            className="mt-4"
                            description={t('store.allowedContentTypesOptionalDescription')}
                        >
                            <SelectContentType
