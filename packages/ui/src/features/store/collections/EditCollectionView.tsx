        ...
        <div className="flex flex-col gap-4 py-2">
            ...
            >
                ...
                <FormItem label={t('type.description')}>
                    <Textarea value={metadata.description} onChange={(e) => setField('description', e.target.value)} />
                </FormItem>
                {!collection.dynamic && (
                    <FormItem
                        label={t('store.allowedContentTypes')}
                        description={t('store.allowedContentTypesSelectDescription')}
                    >
                        <SelectContentType
                ...
                <FormItem
                    label={t('store.tableLayout')}
                    description={t('store.tableLayoutDescription')}
                    className="h-[200px]"
                >
                    <MonacoEditor
                ...
                <FormItem
                    label="Sensitivity"
                    description="BLP sensitivity level — propagated to member documents (max across collections)"
                >
                    <SelectBox
                        options={SecurityLevelLabels.map((label, index) => ({
                            label: `${index} — ${label}`,
                            value: index,
                        }))}
                        value={
                            metadata.sensitivity !== undefined
                                ? {
                                      label: `${metadata.sensitivity} — ${SecurityLevelLabels[metadata.sensitivity] ?? 'Unknown'}`,
                ...
                <FormItem
                    label="Compartments"
                    description="Security compartments — propagated to member documents (union across collections)"
                >
                        ...
                        {metadata.compartments.map((c) => (
                            <Badge
                                ...
                                onClick={() =>
                                    setField(
                                        'compartments',
                                        metadata.compartments.filter((x) => x !== c),
                                    )
                                }
                            >
                                {c} ×
...
function PropertiesEditor({ typeId, collection }: PropertiesEditorProps) {
    ...
    const object = useMemo(
        () => new ManagedObject(schema, (collection.properties as JSONSchemaObject | undefined) || {}),
