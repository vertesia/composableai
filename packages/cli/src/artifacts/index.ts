...
export function registerArtifactsCommand(program: Command) {
    const artifacts = program
        .command('artifacts')
        .description(
            'Manage agent artifacts. Storage ID can be inferred from VERTESIA_ARTIFACT_STORAGE_ID, VERTESIA_AGENTRUN_ID, or VERTESIA_RUN_ID.',
        );
    ...
    artifacts
        ...
        .option(
            '--run-id [runId]',
            'Artifact storage ID (defaults to VERTESIA_ARTIFACT_STORAGE_ID, then VERTESIA_AGENTRUN_ID)',
        )
