...
export function registerObjectsCommand(program: Command) {
    ...
    store
        ...
        .option(
            '--type [type]',
            'The type of the object to create or "auto" to let the application guess the type. If not specified, one can be selected from the list of existing types.',
        )
        .option(
            '--mime [mime]',
            'The mime-type of the file content. If not specified the mime type will be inferred from the file name extension.',
        )
        .option(
            '--path [parentPath]',
            'The path of the parent folder where the object is created. If not specified the object will be created in the root of the store.',
        )
        .option(
            '-r, --recursive',
            'Recurse directory if the file argument is a directory. The default is to not recurse.',
        )
        .action(async (files: string[], options: Record<string, unknown>) => {
            await createObject(program, files, options);
        });
    store
        .command('update <objectId> <type>')
    ...
    store
        ...
        .option(
            '-l,--limit [limit]',
            'Limit the number of objects returned. The default limit is 100. Useful for pagination.',
        )
