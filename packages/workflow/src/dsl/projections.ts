...
export function projectResult<TParams extends object>(
    payload: DSLActivityExecutionPayload<TParams>,
    params: TParams,
    result: unknown,
    fallback: unknown,
) {
    return payload.activity.projection
        ? makeProjection(payload.activity.projection, asRecord(params), result)
        : fallback;
}
