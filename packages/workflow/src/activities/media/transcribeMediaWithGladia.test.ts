...
describe('TranscribeMedia', () => {
    ...
    it('should send transcription request in file mode', async () => {
        ...
        const payload = createTestPayload(params, undefined, {
            url: 'gs://test-bucket/audio.mp3',
            mimetype: 'audio/mpeg',
        });
    ...
    it('should handle missing download URL', async () => {
        ...
        await expect(testEnv.run(transcribeMedia, payload)).rejects.toThrow('Error fetching media URL');
    });
});
