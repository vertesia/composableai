...
export interface InteractionExecutionPayload {
    ...
    /**
     * When true, large media (images, audio, video) generated by the LLM will be stored in blob storage
     * and replaced with storage URIs in the run result to reduce payload size.
     * Necessary for large media outputs, such as 4K, video or multiple images.
     */
...
export interface BaseExecutionRun<P = any> {
    ...
    /**
     * Indicates whether result media for this run was persisted to blob storage instead of inline values.
     */
    store_media_results?: boolean;
}
