...
export function normalizeToolCollection(collection: ToolCollection, vars?: Endpoints): ToolCollectionObject {
    ...
    // Already in object format — substitute URL if needed
    if (vars && collection.url) {
        const substituted = substituteEndpoints(collection.url, vars);
        if (substituted !== collection.url) {
            return { ...collection, url: substituted };
        }
    }
    return collection;
}
