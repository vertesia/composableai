...
const OAUTH_PLATFORM_SCOPES = [OAUTH_SCOPE_PROJECT_SWITCH] as const;

type OAuthStandardScope = (typeof OAUTH_STANDARD_SCOPES)[number];
type OAuthPlatformScope = (typeof OAUTH_PLATFORM_SCOPES)[number];
...
function isOAuthStandardScope(scope: string): scope is OAuthStandardScope {
    return OAUTH_STANDARD_SCOPE_SET.has(scope);
}

function isOAuthPlatformScope(scope: string): scope is OAuthPlatformScope {
