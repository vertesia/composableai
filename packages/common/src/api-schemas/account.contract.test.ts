...
describe('gate 2 — canonical schemas carry stable OpenAPI component references', () => {
    it('publishes the registered component graph without migration-era names', () => {
        const names = Object.keys(ApiSchemaComponents);
        expect(new Set(names).size).toBe(names.length);
        expect(names).toEqual(expect.arrayContaining(['Account', 'AccountBilling', 'StripeBillingStatusResponse']));
        expect(names.some((name) => name.startsWith('Partial_'))).toBe(false);
    });
